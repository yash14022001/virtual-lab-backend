...
Route::group(['middleware' => ['get.menu']], function () {
    ...
    Route::group(['middleware' => ['role:user']], function () {
        ...
        Route::prefix('university')->group(function () {  
            Route::resource('/university', 'UniversityController', ['only' => ['index', 'store', 'destroy', 'update']]);
            Route::post('/university/all-departments', 'UniversityController@getAllDepartmentsFromId')->name("university.getalldepartments");
            Route::resource('/department', 'DepartmentController', ['only' => ['index', 'store', 'destroy', 'update']]);
            Route::post('/department/all-subjects', 'DepartmentController@getAllSubjectsFromId')->name("department.getallsubjects");
            Route::resource('/subject', 'SubjectController', ['only' => ['index', 'store', 'destroy', 'update']]);
        });
        Route::prefix('students')->group(function () {
            Route::resource('/students', 'StudentController', ['only' => ['index', 'store', 'destroy', 'update']]);
            Route::resource('/login-activity', 'LoginActivityController', ['only' => ['index', 'store', 'destroy', 'update']]);
            Route::resource('/inquiry', 'InquiryController', ['only' => ['index', 'destroy']]);
        });
        Route::prefix('practical')->group(function () {  
            Route::resource('/practical', 'PracticalController');
            Route::get('/practical/{id}/excel-sheet', 'ExcelSheetController@index')->name('practical.excel-sheet.index');
            Route::get('/practical/{id}/excel-sheet/download', 'ExcelSheetController@download');
            Route::post('/practical/{id}/excel-sheet/upload', 'ExcelSheetController@upload');
            Route::resource('/inputs', 'InputController', ['only' => ['index', 'store', 'destroy', 'update']]);
            Route::resource('/outputs', 'OutputController', ['only' => ['index', 'store', 'destroy', 'update']]);
        });
...
Route::get('api/university', function(){   return view('dashboard.university.university'); });
